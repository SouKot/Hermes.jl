...
function debug_bottleneck()
    ...
    for i in 1:N
        ...
        new_entity!(world, (Position(pos), Velocity(SVector(0.0f0,0.0f0)), from_agent_params(0.3f0, 80.0f0, v0, 0.5f0)..., Goal(goal_door), Force(SVector(0.0f0,0.0f0))))
    end
    ...
    while t < 2.0f0
        for (entities, pos_col, vel_col, motion_col, goal_col, force_col) in Query(world, (Position{Float32}, Velocity{Float32}, MotionParams{Float32}, Goal{Float32}, Force{Float32}))
            for i in eachindex(pos_col)
                ...
                F_drive = goal_seeking_force(pos_col[i].p, vel_col[i].v, goal_col[i].g, motion_col[i].v_pref, motion_col[i].τ)
