...
"""
Task 24 baseline capture (current setup before hybrid sync runtime implementation).

Captures wall-time + allocation + correctness-facing metrics for:
1) Pure DES M/M/1 (`SimDES.run_mm1!`)
2) Current DES+ABM integration path (headless SimViz ORCA + MM1 events)
3) ABM-only headless ORCA stepping (decomposition baseline)

Run from repo root:
  julia --project=. experiments/scripts/benchmarks/task24_capture_current_baseline.jl
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Case A: MM1-only baseline in current headless integration stack
# ─────────────────────────────────────────────────────────────────────────────

# ─────────────────────────────────────────────────────────────────────────────
# Case B/C: headless SimViz stepping helpers
# ─────────────────────────────────────────────────────────────────────────────
...
function repeat_case(f::Function; reps::Int=5)
...
function main()
    ...
    meta = Dict(
        ...
        "script" => "experiments/scripts/benchmarks/task24_capture_current_baseline.jl",
        "intent" => "Task24 pre-implementation baseline for correctness+performance comparison",
    )

    # Warmup compilation runs (excluded from captured repetitions)
    run_headless_case(with_mm1=true, with_alarm=false, n_agents=0)
    run_headless_case(with_mm1=true, with_alarm=true)
    run_headless_case(with_mm1=false, with_alarm=false)
    ...
    current_des_abm = repeat_case(() -> run_headless_case(
        n_agents=80,
        sim_horizon_s=120.0,
        dt=0.05,
        seed=2026,
    ...
    abm_only = repeat_case(() -> run_headless_case(
        n_agents=80,
        sim_horizon_s=120.0,
        dt=0.05,
        seed=2026,
    ...
    payload = Dict(
        "metadata" => meta,
        "cases" => Dict(
            "mm1_only_headless" => mm1_only,
            "current_des_abm_headless" => current_des_abm,
            "abm_only_headless" => abm_only,
        ),
        "comparison" => Dict(
            "des_abm_vs_mm1_only_elapsed_ratio_mean" =>
                current_des_abm["summary"]["elapsed_s"]["mean"] /
                mm1_only["summary"]["elapsed_s"]["mean"],
            "des_abm_vs_abm_only_elapsed_ratio_mean" =>
                current_des_abm["summary"]["elapsed_s"]["mean"] /
                abm_only["summary"]["elapsed_s"]["mean"],
            "des_abm_vs_abm_only_alloc_ratio_mean" =>
                current_des_abm["summary"]["alloc_bytes"]["mean"] /
                abm_only["summary"]["alloc_bytes"]["mean"],
        ),
    )

    toml_path = joinpath(out_dir, "task24_current_baseline_$(timestamp).toml")
    open(toml_path, "w") do io
        TOML.print(io, payload)
    end

    latest_path = joinpath(out_dir, "task24_current_baseline_latest.toml")
    cp(toml_path, latest_path; force=true)

    println("\nTask 24 current baseline captured.")
    println("  -> ", toml_path)
    println("  -> ", latest_path)

    println("\nKey means:")
    println("  mm1_only_headless.elapsed_s = ", round(mm1_only["summary"]["elapsed_s"]["mean"], digits=4))
    println("  current_des_abm.elapsed_s = ", round(current_des_abm["summary"]["elapsed_s"]["mean"], digits=4))
    println("  abm_only.elapsed_s = ", round(abm_only["summary"]["elapsed_s"]["mean"], digits=4))
    println("  des_abm/mm1_only elapsed ratio = ", round(payload["comparison"]["des_abm_vs_mm1_only_elapsed_ratio_mean"], digits=4))
    println("  des_abm/abm_only elapsed ratio = ", round(payload["comparison"]["des_abm_vs_abm_only_elapsed_ratio_mean"], digits=4))
end
